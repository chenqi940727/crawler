...
public class IqyTVCrawlerCustomer implements Runnable {
    ...
    public void run() {
        ...
        Boolean flag = true;
        while(flag){
            if(redisDao.exists(RedisKey.IQY_TV_ALBUMID_LIST) && redisDao.getListSize(RedisKey.IQY_TV_ALBUMID_LIST) > 0){
                Long startTime = System.currentTimeMillis();
                String albumId = (String) redisDao.lPop(RedisKey.IQY_TV_ALBUMID_LIST);
                try {
