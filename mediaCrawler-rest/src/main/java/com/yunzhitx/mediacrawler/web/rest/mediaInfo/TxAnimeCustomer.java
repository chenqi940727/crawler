 ...
 * ---------     -------------   --------------------------------------
 **/
public class TxAnimeCustomer implements Runnable{
    ...
    public void run() {
        ...
        Boolean flag = true;
        while(flag){
            if(redisDao.exists(RedisKey.TX_ANIME_ALBUMID_LIST) && redisDao.getListSize(RedisKey.TX_ANIME_ALBUMID_LIST) > 0){
                Long startTime = System.currentTimeMillis();
                String tvId = (String) redisDao.lPop(RedisKey.TX_ANIME_ALBUMID_LIST);
                try {
                    TXUtils.addAnimeMedia(tvId, restTemplate);
